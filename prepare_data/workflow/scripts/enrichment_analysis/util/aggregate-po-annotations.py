...
def save_annotations_to_csv(annotations_df, output_dir):
    ...
    annotations_df.to_csv(
        f"{output_dir}/po-annotations.tsv", sep="\t", index=False, header=False
    )
    print(f"Generated {output_dir}/po-annotations.tsv")


def save_id_po_names_to_csv(id_to_names_df, output_dir):
    id_to_names_df.to_csv(
        f"{output_dir}/po-id-to-name.tsv", sep="\t", index=False, header=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_dir",
        help="output directory for the TSV file mapping MSU accessions to TO term IDs",
    )
