...
Module_detection_algo = namedtuple(
    "Module_detection_algo", ["multiplier", "default_param", "low", "high"]
)
module_detection_algos = {
    ...
    "fox": Module_detection_algo(100, 0.05, "1 (Looser Modules)", "4 (Denser Modules)"),
}
...
COEXPRESSION_NETWORKS_VALUE_LABEL = [
    ...
    {
        "value": "RCRN",
        "label": "Rice Combined Mutual Ranked Network (RCRN)",
        "label_id": "rcrn",
    },
]
...
enrichment_tabs = [
    ...
    Enrichment_tab("Pathways (Over-Representation)", "pathway_enrichment/ora"),
...
def create_module_enrichment_results_dir(
...
def do_module_enrichment_analysis(
    implicated_gene_ids, genomic_intervals, addl_genes, network, algo, parameters
):
    """
    ...
    if not path_exists(ENRICHED_MODULES_PATH):
        ...
        with open(MODULES_PATH) as modules_file, open(
            ENRICHED_MODULES_PATH_WITH_TIMESTAMP, "w"
        ) as enriched_modules_file:
            modules = []
            ...
            for idx, line in enumerate(modules_file):
                if idx in modules:
                    ...
                    p_values.append(fisher_exact(table, alternative="greater").pvalue)
            ...
            significant_adj_p_values = [
                ...
                for idx, adj_p_value in enumerate(adj_p_values)
                if adj_p_value < Constants.P_VALUE_CUTOFF
            ]
        ...
        try:
            os.replace(ENRICHED_MODULES_PATH_WITH_TIMESTAMP, ENRICHED_MODULES_PATH)
        except:
            pass
...
def construct_contigency_table(background_genes, implicated_genes, module_genes):
    ...
    in_implicated_not_in_module = len(implicated_genes.intersection(not_in_module))

    not_in_implicated_in_module = len(not_in_implicated.intersection(module_genes))
    not_in_implicated_not_in_module = len(not_in_implicated.intersection(not_in_module))

    table = [
        [in_implicated_in_module, not_in_implicated_in_module],
...
def add_link_to_genes(gene_ids_str):
    return "\n".join(
        [
            get_msu_browser_link_single_str(gene_id)
            for gene_id in gene_ids_str.split("\n")
        ]
    )
...
def convert_transcript_to_msu_id(transcript_ids_str, network):
    ...
    """
    with open(
        f"{Constants.MSU_MAPPING}/{network}/transcript-to-msu-id.pickle", "rb"
    ) as f:
...
def convert_rap_to_msu_id(rap_ids_str, network):
    """
    Converts given RAP IDs to their respective MSU accessions.

    BUGFIX (PPI): the Pathways (Over-Representation) and Pathway-Express
    analyses operate on RAP-labeled module/gene lists (see
    convert_modules_msu_to_rap), not KEGG-transcript-labeled ones like SPIA --
    they were previously (incorrectly) run through convert_transcript_to_msu_id,
    which looks up a per-network, list-valued transcript->MSU pickle. Since
    rap_to_msu.pickle is a single, global, single-valued (RAP -> one MSU gene,
    or empty string if unmapped) crosswalk, reusing that function silently
    produced no matches and every result row was dropped downstream. This is
    a dedicated function for that different shape of mapping.

    Parameters:
    - rap_ids_str: RAP IDs, newline-separated
    - network: Coexpression/PPI network (kept for call-signature parity with
      convert_transcript_to_msu_id; the RAP->MSU crosswalk itself is global)

    Returns:
    - Equivalent MSU accessions of the RAP IDs, formatted for display
    """
    with open(f"{Constants.MSU_MAPPING}/rap_to_msu.pickle", "rb") as f:
        mapping_dict = pickle.load(f)

    output_str = ""
    rap_ids = rap_ids_str.split("\n")
    for rap_id in rap_ids:
        msu_id = mapping_dict.get(rap_id, "")
        if not msu_id:
            continue
        output_str += f'{get_msu_browser_link_single_str(msu_id)}\n<span class="small">({rap_id})</span><br><br>'

    if not output_str:
        # Nothing mapped -- fall back to showing the raw RAP IDs rather than
        # silently returning an empty string
        return rap_ids_str.replace("\n", "<br>")
...
def get_genes_in_module(module_idx, network, algo, parameters):
    with open(
        f"{Constants.NETWORK_MODULES}/{network}/transcript/{algo}/{parameters}/{algo}-module-list.tsv"
    ) as f:
...
def get_genes_in_module_and_pathway(pathway_id, module_idx, network, algo, parameters):
    return "\n".join(
        list(
            get_genes_in_pathway(pathway_id, network).intersection(
...
def convert_to_df_go(result):
    ...
    result["Genes"] = result.apply(lambda x: add_link_to_genes(x["Genes"]), axis=1)

    result["ID"] = get_go_link(result, "ID")

    result = result.sort_values("Adj. p-value")

    display_cols_in_sci_notation(result, [col for col in cols if "p-value" in col])
...
def convert_to_df_ora(result, network):
    ...
    result["Genes"] = result.apply(
        lambda x: convert_rap_to_msu_id(x["Genes"], network), axis=1
    )
...
def convert_to_df_pe(result, module_idx, network, algo, parameters):
    ...
    result["Genes"] = result.apply(
        lambda x: get_genes_in_module_and_pathway(
            x["ID"], module_idx, network, algo, parameters
        ),
        axis=1,
    )
...
def convert_to_df_spia(result, network):
    ...
    # Prettify display of genes and convert to MSU accessions
    result["Genes"] = result["View on KEGG"].apply(get_genes_from_kegg_link)
...
def convert_to_df(active_tab, module_idx, network, algo, parameters):
    ...
    columns = {
        ...
        "pe": [
            ...
            "ORA p-value",
            "Combined p-value",
            "Adj. Perturbation p-value",
            "Adj. Accumulation p-value",
            "Adj. ORA p-value",
            "Adj. Combined p-value",
        ],
        "spia": [
            ...
            "Combined p-value",
            "Adj. Combined p-value",
            "Adj. Combined p-value (Bonferroni)",
            "Pathway Status",
            "View on KEGG",
        ],
    }
    ...
    try:
        result = pd.read_csv(
            file, delimiter="\t", names=columns[enrichment_type], skiprows=1
        )
    ...
    with open(
        ...
        for idx, module in enumerate(modules):
            if idx == module_idx - 1:
                ...
                return len(module_genes), len(
                    set.intersection(set(module_genes), set(implicated_genes))
                )
...
def get_rapdb_entry(gene, rapdb_mapping):
    if rapdb_mapping[gene]:
        return html.Ul(
            [
                html.Li(get_rapdb_single_str(entry, dash=True))
                for entry in sorted(rapdb_mapping[gene])
                if entry
            ],
...
def get_uniprot_entry(gene, gene_descriptions_mapping):
    try:
        return get_uniprot_link_single_str(
            gene_descriptions_mapping[gene][1], dash=True
        )
    except KeyError:
        return NULL_PLACEHOLDER
...
def get_pfam_entry(gene, pfam_mapping, iric_mapping):
    try:
        return html.Ul(
            [
                html.Li(get_pfam_link_single_str(entry[1], entry[0], dash=True))
                for entry in sorted(pfam_mapping[iric_mapping[gene]])
                if entry[1]
            ],
            className="no-bottom-space",
        )
...
def get_interpro_entry(gene, interpro_mapping, iric_mapping):
    try:
        return html.Ul(
            [
                html.Li(get_interpro_link_single_str(entry[1], entry[0], dash=True))
                for entry in sorted(interpro_mapping[iric_mapping[gene]])
                if entry[1]
            ],
            className="no-bottom-space",
        )
    ...
    try:
        ...
        for character_major in sorted(mapping[gene]):
            ...
            for character_minor in sorted(mapping[gene][character_major]):
                pubs = [
                    html.Li(get_doi_link_single_str(pub, dash=True))
                    for pub in sorted(mapping[gene][character_major][character_minor])
                ]
                character_minors_list.append(html.Li([character_minor, html.Ul(pubs)]))
...
def get_pubmed_entry(gene, pubmed_mapping):
    try:
        return html.Ul(
            [
                html.Li(get_pubmed_link_single_str(pubmed_id[0], dash=True))
                for pubmed_id in sorted(
